...
function agregarAlCarrito(idCarrito, idProducto) {
    return apiCars.postProd(idCarrito, idProducto).then(() => {
        actualizarListaCarrito(idCarrito)
    })
}
...
function quitarDelCarrito(idProducto) {
    const idCarrito = document.getElementById('comboCarritos').value
    return apiCars.deleteProd(idCarrito, idProducto).then(() => {
        actualizarListaCarrito(idCarrito)
    })
}

function actualizarListaCarrito(idCarrito) {
    return apiCars.getProds(idCarrito)
    ...
    if (productos.length > 0) {
        ...
        for (const prod of productos) {
            html += `
                    ...
                    <td><a type="button" onclick="quitarDelCarrito('${prod.id}')"><img src="https://cdn3.iconfinder.com/data/icons/social-messaging-ui-color-line/254000/82-256.png" width="48px"> Borrar</a></td>
                    </tr>`
        }
        html += `
            </table>
